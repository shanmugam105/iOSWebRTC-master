...
class ChatViewController: UIViewController {
    ...
    func setupFirebase() {
        let dbReferance = Database.database().reference()
        self.observerSignalRef = dbReferance.child("Call/\(receiver)")
        self.offerSignalRef    = dbReferance.child("Call/\(sender)")
    ...
    func observerSingnal() {
        ...
        self.observerSignalRef?.observe(.value) { [weak self] (snapshot) in
            ...
            switch type {
            case "offer":
                // offer processing when you receive
                LOG("Received offer ...")
            ...
            case "answer":
                // answer processing when you receive
                LOG("Received answer ...")
